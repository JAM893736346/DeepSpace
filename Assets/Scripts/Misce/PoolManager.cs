...
public class PoolManager : MonoBehaviour
{
    ...
    //清除全场子弹
    public void ClearEnemyProjectilePool()
    {
        foreach (Pool item in enemyProjectilePools)
        {
            for (int i = 0; i < item.parent.childCount; i++)
            {
                if (item.parent.GetChild(i).gameObject.activeSelf)
                {
                    PoolManager.Release(DestoryVFX,item.parent.GetChild(i).position);
